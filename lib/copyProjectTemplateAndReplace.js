...
const copyAndReplace = require("./copyAndReplace");
...
function copyProjectTemplateAndReplace(
  ...
  walk(srcPath).forEach(absoluteSrcFilePath => {
    ...
    const safeProjectName = newProjectName.replace(/-/g, "_");
    const relativeRenamedPath = dotFilePath(relativeFilePath)
      .replace(/rnswift_template/g, safeProjectName)
      .replace(/rnswifttemplate/g, newProjectName);
    ...
    copyAndReplace(
      ...
      {
        "RNSwift DisplayName": options.displayName || newProjectName,
        rnswifttemplate: newProjectName,
        rnswift_template: safeProjectName
...
function upgradeFileContentChangedCallback(
  ...
  if (contentChanged === "new") {
    ...
    throw new Error(
      `Unkown file changed state: ${relativeDestPath}, ${contentChanged}`
    );
  }
}
